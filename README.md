...
- **实用工具箱**：  
  ...
  - **计算器**：进行数学运算（支持复杂表达式）。  
  - **时间查询**：获取当前时间、日期或时区信息。
  - **LangGraph架构**：基于 LangGraph 构建灵活的 Agent 状态图，实现工具调用、记忆管理与多步骤推理的自动编排。  
...
- **Agent 框架**：LangGraph（状态图管理、工具路由、循环控制）  
...
本项目使用 **LangGraph** 构建可扩展的 Agent 循环，核心流程如下：  
状态图（StateGraph）


- **状态对象**：包含对话历史、当前消息、工具调用结果、长期记忆上下文等。  
- **节点（Nodes）**：  
  - `agent`：调用 DeepSeek 模型，决策是否需要工具或直接回复。  
  - `tools`：执行选中的工具（天气、计算、时间、搜索等）。  
  - `memory`：检查并更新长期记忆（提取用户偏好）。  
- **边（Edges）**：根据模型输出动态路由，支持条件循环（如工具调用后返回 agent 再次推理）。  

工作流程


1. 用户输入 → 更新状态中的 `messages`。  
2. `agent` 节点 → 调用 LLM，传入工具定义和状态。  
3. 若模型返回 `tool_calls` → 跳转到 `tools` 节点执行函数，结果追加到状态，然后返回 `agent`。  
4. 若模型返回最终回复 → 跳转到 `memory` 节点，分析并保存长期记忆。  
5. 最终回复输出给用户。  

自定义扩展


- 新增工具：在 `tools.py` 中定义函数，在 `main.py` 的 `tools` 列表中注册。  
- 修改记忆提取逻辑：调整 `memory` 节点的提示词或分析算法。  
> 此架构借鉴了 LangGraph 官方 Agent 示例，并适配了天音的人设和记忆机制。  
